    ...
    def _get_individual_parameters(self, model: AbstractModel, dataset: Dataset):
        ...
        with self._device_manager(model, dataset):
            ...
            realizations.initialize_individuals(
                dataset.n_individuals, model, init_at_mean=True,
            )
            ind_vars_names = copy(realizations.individual_names)
            ...
            for self.current_iteration in range(1, n_iter+1):
                ...
                if not self._is_burn_in():
                    realizations_history.append(clone_realizations(realizations))
            ...
            torch_realizations = {
                ind_var_name: torch.stack(
                    [
                        realizations.get_tensor_by_name(ind_var_name)
                        for realizations in realizations_history
                    ]
                ) for ind_var_name in realizations.individual_names
            }
