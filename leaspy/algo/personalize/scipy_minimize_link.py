...
class ScipyMinimizeLink(AbstractPersonalizeAlgo):
    """
    ...
    def _pull_individual_parameters(self, x, model, cofactors):
        """
    ...
    def obj(self, x, *args):
        ...
        *args
        ...
        model, times, values, cofactors, with_gradient = args

        ## Attachment term
        individual_parameters = self._pull_individual_parameters(x, model, cofactors)
    ...
    def _get_individual_parameters_patient(self, model, times, values, cofactors, *, patient_id=None):
        """
        ...
        res = minimize(self.obj,
                       jac=with_jac,
                       x0=initial_value,
                       args=(model, times.unsqueeze(0), values, cofactors, with_jac),
                       **self.minimize_kwargs
                       )

        individual_params_f = self._pull_individual_parameters(res.x, model, cofactors)
    ...
    def _get_individual_parameters_patient_master(self, it, data, model, *, patient_id=None):
        ...
        cofactors = data.get_cofactors_patient(it)

        individual_params_tensorized, err = self._get_individual_parameters_patient(model, times, values, cofactors, patient_id=patient_id)
        ...
        return {k: v.item() if (k != 'sources' and k != 'g' and k != 'v0') else v.detach().squeeze(0).tolist() for k,v in individual_params_tensorized.items()}
