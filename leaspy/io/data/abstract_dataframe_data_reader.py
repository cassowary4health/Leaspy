...
class AbstractDataframeDataReader:
    ...
    def _clean_numeric_data(self, df: pd.DataFrame, drop_full_nan: bool, warn_empty_column: bool) -> pd.DataFrame:
        """
        ...
        types_nok = {ft: dtype for ft, dtype in df.dtypes.items() if not self._check_numeric_type(dtype)}
        if types_nok:
    ...
    def read(self, df: pd.DataFrame, *, drop_full_nan: bool = True, sort_index: bool = False,
             warn_empty_column: bool = True) -> None:
        """
        ...
        # Clean visit data
        df = self._clean_dataframe(df, drop_full_nan=drop_full_nan, warn_empty_column=warn_empty_column)
