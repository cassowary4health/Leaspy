...
class FitOutputManager:
    ...
    def save_realizations(self, iteration: int, realizations: CollectionRealization) -> None:
        ...
        for name in ("xi", "tau"):
            value = realizations.get_tensor_by_name(name).squeeze(1).detach().tolist()
            ...
            with open(path, 'a', newline='') as filename:
                ...
                writer.writerow([iteration] + value)
        if "sources" in realizations.individual_names:
            for i in range(realizations.get_tensor_by_name("sources").shape[1]):
                value = realizations.get_tensor_by_name("sources")[:, i].detach().tolist()
    ...
    def plot_patient_reconstructions(
        ...
        self.plotter.plot_patient_reconstructions(
            path_iteration, data, model, realizations.individual_tensors_dict, **self.plot_options
        )
