...
class Plotter:
    ...
    def plot_mean_trajectory(self, model, **kwargs):
        ...
        try:
            ...
            timepoints = torch.tensor(timepoints, dtype=torch.float32, device=model.device).unsqueeze(0)
            ...
            timepoints = np.linspace(model[0].parameters['tau_mean'] - 3 * np.sqrt(model[0].parameters['tau_std']),
                                     ...
                                     100)
            timepoints = torch.tensor(timepoints, dtype=torch.float32, device=model[0].device).unsqueeze(0)
    ...
    def plot_patient_trajectory(self, model, results, indices, **kwargs):
        ...
        for idx in indices:
            ...
            t = torch.tensor(timepoints, dtype=torch.float32, device=model.device).unsqueeze(0)
    ...
    def plot_patients_mapped_on_mean_trajectory(self, model, results):
        ...
        for i in range(dataset.values.shape[-1]):
            ...
            for idx in range(50):
                ax.plot(reparametrized_time[idx, 0:dataset.nb_observations_per_individuals[idx]].cpu().detach().numpy(),
                        dataset.values[idx, 0:dataset.nb_observations_per_individuals[idx], i].cpu().detach().numpy(), 'x', )
                ax.plot(reparametrized_time[idx, 0:dataset.nb_observations_per_individuals[idx]].cpu().detach().numpy(),
                        patient_values[idx, 0:dataset.nb_observations_per_individuals[idx], i].cpu().detach().numpy(),
                        alpha=0.8)
            if 'logistic' in model.name:
                plt.ylim(0, 1)
    ...
    def plot_error(path, dataset, model, param_ind, colors=None, labels=None):
        ...
        for i in range(dataset.values.shape[-1]):
            ...
            for idx in range(patient_values.shape[0]):
                err[i].extend(dataset.values[idx, 0:dataset.nb_observations_per_individuals[idx], i].cpu().detach().numpy() -
                              patient_values[idx, 0:dataset.nb_observations_per_individuals[idx], i].cpu().detach().numpy())
    ...
    def plot_patient_reconstructions(path, data, model, param_ind, max_patient_number=10, attribute_type=None):
        ...
        min_time, max_time = np.percentile(data.timepoints[data.timepoints > 0.0].cpu().detach().numpy(), [10, 90])
