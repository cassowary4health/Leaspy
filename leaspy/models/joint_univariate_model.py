...
class JointUnivariateModel(AbstractModel, OrdinalModelMixin):
    ...
    **kwargs
    ...
    def compute_individual_tensorized_hazard(self, timepoints, individual_parameters, *, attribute_type=None):
        ...
        # Individual parameters
        xi = individual_parameters['xi'].reshape(timepoints.shape)
        reparametrized_time = (torch.exp(xi-self.parameters['xi_mean']))*timepoints
    ...
    def compute_individual_attachment_event(self, data, param_ind):

        survival = self.compute_individual_tensorized_survival(data.event_time_min, param_ind)
        hazard = self.compute_individual_tensorized_hazard(data.event_time_max, param_ind)
    ...
    def compute_individual_attachment_tensorized(self, data, param_ind: DictParamsTorch, *,
        ...
        attachment_events = -1*torch.log(self.compute_individual_attachment_event(data, param_ind))
        attachment_total = attachment_visits + attachment_events
