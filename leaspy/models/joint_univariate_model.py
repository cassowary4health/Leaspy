...
class JointUnivariateModel(ABC):
    ...
    **kwargs
    ...
    ## --- DATA ATTACHMENT CALLIBRATION FUNCTIONS ---
    ...
    ## --- DATA ATTACHMENT PERSONNALIZATION FUNCTIONS ---
    def compute_perso_attachment_visits(self, timepoints, values, individual_parameters: DictParams, *,
                                      skip_ips_checks: bool = False):
        """
        Compute scores values at the given time-point(s) given a subject's individual parameters.
    ...
    def compute_perso_grad_attachment_visits(self, timepoints, values, individual_parameters, *, attribute_type=None):
        ...
        to_cat = [
                derivatives['xi'] * self.parameters['xi_std'],
                derivatives['tau'] * self.parameters['tau_std']
            ]
    ...
    def compute_sufficient_statistics(self, data, realizations, iteration=None):
        ...
        if self.noise_model in ['joint']:
            sufficient_statistics['events_likelihood'] = self.compute_individual_attachment_events(data,
                                                                                                   individual_parameters,
                                                                                                   attribute_type='MCMC').sum()
            sufficient_statistics['visits_likelihood'] = self.compute_individual_attachment_visits(data,
                                                                                                   individual_parameters,
                                                                                                   attribute_type='MCMC').sum()
    ...
    def compute_sum_squared_tensorized(self, dataset, param_ind, *,
        ...
        """
        L2_res_per_ind_per_ft = self.compute_sum_squared_per_ft_tensorized(dataset, param_ind,
                                                                           attribute_type=attribute_type)
