...
class AbstractManifoldModelLinkAttributes(AbstractAttributes):
    """
    ...
    def __init__(self, name: str, dimension: int, source_dimension: int = None, device: torch.device = None):
        ...
        self.link: torch.FloatTensor = None
        ...
        if self.univariate:
            ...
            self.update_possibilities = ('all', 'g', 'link', 'betas')
    ...
    def _compute_Q_indiv(self, dgamma_t0, G_metric, strip_col=0):
        """
        Householder decomposition, adapted for a non-Euclidean inner product defined by:
        (1) :math:`< x, y >Metric(p) = < x, G(p) y >Eucl = xT G(p) y`, where:
        :math:`G(p)` is the symmetric positive-definite (SPD) matrix defining the metric at point `p`.
