...
import torch
from src.algo.fit.abstract_mcmc import AbstractFitMCMC
import numpy as np
...
class HMC_SAEM(AbstractFitMCMC):
    ...
    def _leapfrog_step(self, p, realizations, model, data):
        ...
        for key in p.keys():
            ...
            with torch.no_grad():
                if key=='tau':
                    realizations[key].tensor_realizations += self.algo_parameters['eps'] * p[key]*(10**2)  ## for all i, m_i=1, to be changed some day
                else:
    ...
    def _hmc_sample_individual_realizations(self, data, model, realizations):
        ...
        for key in realizations.reals_ind_variable_names:
            ...
            with torch.no_grad():
                realizations[key].tensor_realizations = realizations[key].tensor_realizations*accepted+(1.-accepted)*old_real[key].tensor_realizations
